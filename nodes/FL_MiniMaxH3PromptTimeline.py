...
def _beat_shot_sections(schedule):
    ...
    for index, (section, raw) in enumerate(zip(sections, raw_sections), 1):
        ...
        if section["crossfade_end"] > section["crossfade_start"] + _EPS:
            raise ValueError(
                ...
                f"Remove the crossfade from section {index} before beat-shot planning."
            )
        render_group = raw.get("render_group")
        if (
            render_group is not None
            and (
                isinstance(render_group, bool)
                or not isinstance(render_group, int)
                or render_group < 1
            )
        ):
            raise ValueError(
                f"FL MiniMax H3 Beat Shot Planner section {index} has an invalid render group."
            )
        resolved.append({
            **section,
...
def _render_section_groups(sections):
    positions = {}
    for index, section in enumerate(sections):
    ...
    for group, indices in positions.items():
        if indices != list(range(indices[0], indices[-1] + 1)):
            raise ValueError(
                f"FL MiniMax H3 Beat Shot Planner render group {group} must contain "
                "consecutive prompt sections."
            )
    ...
    for section in sections:
        ...
        if (
            group is not None
            and groups
            and groups[-1][0].get("render_group") == group
        ):
...
class FL_MiniMaxH3BeatShotPlanner(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            category="FL/MiniMax H3/Prompting",
            description=(
                "Turns an FL beat prompt schedule into MiniMax H3 renders. Prompt sections "
                "grouped in the audio sequencer share one render; ungrouped sections remain independent."
            ),
            inputs=[
                io.Clip.Input(
                    "clip",
                    tooltip="MiniMax H3 text encoder used for each planned render.",
                ),
                io.Vae.Input(
                    "vae",
                ...
                io.Int.Input(
                    ...
                    max=nodes.MAX_RESOLUTION,
                    step=32,
                    tooltip="Width of every planned render.",
                ),
                io.Int.Input(
                    "height",
                    default=768,
                    ...
                    max=nodes.MAX_RESOLUTION,
                    step=32,
                    tooltip="Height of every planned render.",
                ),
                io.Combo.Input(
                    "affect_audio",
                    options=["video only", "video and audio"],
                ...
                io.Autogrow.Input(
                    "ref_images",
                    optional=True,
                    tooltip="Character or scene images applied to every planned render.",
                    template=io.Autogrow.TemplatePrefix(
                ...
                io.Autogrow.Input(
                    "ref_videos",
                    optional=True,
                    tooltip="Reference videos applied to every planned render.",
                    template=io.Autogrow.TemplatePrefix(
    ...
    def execute(
        ...
        for index, section_group in enumerate(section_groups):
            ...
            for section in section_group:
                local_start = (section["start_frame"] - start_frame) / minimax_h3.FPS
                local_end = (section["end_frame"] - start_frame) / minimax_h3.FPS
                fade_in = max(0.0, section["fade_in_end"] - section["start"])
                fade_out = max(0.0, section["end"] - section["fade_out_start"])
                local_sections.append({
                    **section,
                    "start": local_start,
                    "end": local_end,
                    "fade_in_end": min(local_end, local_start + fade_in),
                    "fade_out_start": max(local_start, local_end - fade_out),
                    "crossfade_start": local_start,
                    "crossfade_end": local_start,
                })
            authored_sections = [dict(section) for section in local_sections]
            local_sections, boundary_adjustments, extended_final_section = _align_h3_sections(
                local_sections,
                authored_frames,
                render_frames,
                video.shape[2],
        ...
        logging.info(
            "FL MiniMax H3 beat shot planner: %d prompt sections into %d planned renders, "
            "%d authored frames, %d render frames, %d padding frames.",
            len(sections),
            len(shots),
            total_frames,
            total_render_frames,
